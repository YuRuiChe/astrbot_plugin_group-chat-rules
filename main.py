...
@register("astrbot_plugin_group-chat-rules", "语芮澈", "可以判断群规是否适合当前场景", "v14", "https://github.com/YuRuiChe/astrbot_plugin_group-chat-rules")
class MyPlugin(Star):
    ...
    async def on_message(self, event: AstrMessageEvent):
        ...
        if not message:
            return
        if message.startswith("你的发言违反了群规") or "你的发言违反了群规" in message:
            return
        try:
            # 获取配置文件选择的LLM提供商ID
            provider_id = self.llm_provide
            ...
            llm_response = await self.context.llm_generate(
                chat_provider_id=provider_id,
                prompt=message,
                system_prompt=f"群规如下“{self.is_regulations}” 判断此言论“{message}”是否符合群规，如果违反，请输出“Y”这个字母；如果没有违反，那就输出“N”这个字母，坚决不要有多余的输出。"
            )
            # 获取回复内容
            reply = llm_response.completion_text
            # 发送回复
